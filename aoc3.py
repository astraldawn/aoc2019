...
def get_path_coords(path):
    path_coords = [(0, 0)]
    c_x, c_y = path_coords[0]
    for instruction in path:
        direction, distance = instruction[0], int(instruction[1:])
        if direction == 'U':
            path_coords.extend([(c_x, c_y + i) for i in range(1, distance + 1)])
        if direction == 'D':
            path_coords.extend([(c_x, c_y - i)
                                for i in range(1, distance + 1)])
        if direction == 'L':
            path_coords.extend([(c_x - i, c_y)
                                for i in range(1, distance + 1)])
        if direction == 'R':
            path_coords.extend([(c_x + i, c_y)
                                for i in range(1, distance + 1)])
        c_x, c_y = path_coords[-1]
...
def find_intersection_point(p1, p2):
    ...
    p1_coords = get_path_coords(p1)
    p2_coords = get_path_coords(p2)

    p1_p2_intersections = set(p1_coords).intersection(set(p2_coords))
    p1_p2_intersections = [p1_coords.index((x, y)) + p2_coords.index((x, y)) for x, y in p1_p2_intersections]
    answer = min(filter(lambda x: x != 0, p1_p2_intersections))
    return answer


for path_a, path_b in zip(paths_a, paths_b):
    print(find_intersection_point(path_a, path_b))
